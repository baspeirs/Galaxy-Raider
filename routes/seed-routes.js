...
module.exports = function(app) {
  ...
  app.get("/seeds/races", (req, res) => {
    db.sequelize
      ...
      .then(() => {
        db.Race.findAll({
          include: [
            { model: db.Planet, as: "Planet" },
            { model: db.Planet, as: "Hostile" }
          ]
        });
      })
      .then(data => res.json(data));
  ...
  app.get("/seeds/characters", (req, res) => {
    db.sequelize
      .query("SET FOREIGN_KEY_CHECKS = 0")
      .then(() => db.Character.sync({ force: true }))
      .then(() => db.sequelize.query("SET FOREIGN_KEY_CHECKS = 1"))
      .then(() => {
        return db.Character.bulkCreate(require("../db/seeds/characters"));
      })
      .then(() => db.Character.findAll({}))
      .then(data => res.json(data));
  });
};
